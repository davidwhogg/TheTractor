...
\begin{eqnarray}\displaystyle
Q_{\exp}(\spos) &\approx& \frac{1}{Z_{\exp}}\,\exp(-\frac{|\spos|}{\xi_{\exp}})
\\
Z_{\exp} &=& X.XXXX
\\
\xi_{\exp} &=& X.XXXX
...
and $\xi_{\exp}$ are parameters that ensure that the profile integrates
...
\begin{eqnarray}\displaystyle
Q_{\dev}(\spos) &\approx& \frac{1}{Z_{\dev}}\,\exp(-\left[\frac{|\spos|}{\xi_{\dev}}\right]^{1/4})
\\
Z_{\dev} &=& X.XXXX
\\
\xi_{\dev} &=& X.XXXX
