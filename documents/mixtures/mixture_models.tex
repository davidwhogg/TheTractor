...
\cite{ciotti} give an equation for $\alpha^{\ser(n)}$ in their
equation 5:
\begin{eqnarray}
\gamma(2 n, \alpha^{\ser(n)}) &=& \frac{\Gamma(2 n)}{2}
\end{eqnarray}
where $\gamma$ is the \emph{left} or \emph{lower} incomplete gamma function,
\begin{eqnarray}
\gamma(a, x) &=& \int_0^x \exp(-t) \, t^{a-1} \, \mathrm{d} t \quad .
\end{eqnarray}
Fortunately for us, this can be evaluated using
\begin{eqnarray}
\alpha^{\ser(n)} &=& \mathrm{gammaincinv}(2 n, \, \frac{1}{2})
\end{eqnarray}
using the gammaincinv (inverse incomplete gamma) function in the
\project{scipy.special} library.
