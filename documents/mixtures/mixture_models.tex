...
% to-do
% -----
% - DSTN: we need *some* estimate of the speed-up this delivers to The Tractor in the abstract and in the text -- of course this is bullshit because we get to decide how precisely we were doing things prior to using the mixtures -- but we need it since it is our main point.
...
affine-transform.  Most importantly, if you have a MoG PSF model
(which we advocate), the PSF-convolved, affine-transformed galaxy
...
projects by XXX; they ought to make any code faster at very little
cost in accuracy.
\end{abstract}
...
fits by more than do these high-quality MoG approxmations; no
precision is lost.
...
require that the user to \emph{two} convolutions, the first withe the
...
very large... DSTN: How much did we benefit when we switched to MoGs
in \project{The Tractor}?...
...
approximations are are not that interesting in themselves.  However,
